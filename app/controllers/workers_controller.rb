class WorkersController < ApplicationController
  def show
    ...
    @count_day = @customers.where(status:"draft").where.not(tel: nil).joins(:versions).where(versions: { whodunnit: current_worker.id }).where('customers.updated_at >= ?', Time.current.beginning_of_day).where('customers.updated_at <= ?', Time.current.end_of_day).count
