...
class OkuriteController < ApplicationController
  ...
  def index
    ...
    if params[:q]&.dig(:contact_trackings_id_in).present?
      ...
      @customers = Customer.joins(:contact_trackings)
                           .where(contact_trackings: { id: params[:q][:contact_trackings_id_in], sender_id: @sender.id })
  ...
  def create
    ...
    if params[:status] == '送信済'
       # Only mark as "sent" if we have actual verification
       if params[:verification] == 'success'
      attributes[:sended_at] = Time.current
      attributes[:verification_data] = params[:verification_data]
   else
     # Mark as suspicious if claimed sent but no verification
    attributes[:status] = '送信状態不明'
    attributes[:error_reason] = params[:error_reason] || '未検証の送信'
    end
  end   
...
")
  ...
  def autosettings
    ...
    @customers.each do |cust|
      ...
      if contact_tracking.save
        ...
        begin
          # Determine platform for cross-compatibility
          platform = RUBY_PLATFORM.include?('darwin') ? 'mac' : 
                     RUBY_PLATFORM.include?('linux') ? 'linux' : 'windows'
          
          # Sidekiqにジョブを登録 with platform info
          if parsed_scheduled_date <= Time.zone.now
            AutoformSchedulerWorker.perform_async(contact_tracking.id, 
                                                verification_required: true, 
                                                platform: platform)
          else
            AutoformSchedulerWorker.perform_at(parsed_scheduled_date, 
                                             contact_tracking.id, 
                                             verification_required: true, 
                                             platform: platform)
          end
  ...
  # NEW: Success rate dashboard for client requirement (60%+ success rate)
  def statistics
    @sender = Sender.find(params[:sender_id])
    
    # Get all submission attempts for this sender
    all_submissions = ContactTracking.for_sender(@sender.id)
    
    # Calculate metrics
    @total_attempts = all_submissions.count
    @successful_submissions = all_submissions.where(status: '送信済').count
    @pending_submissions = all_submissions.where(status: '自動送信予定').count
    @failed_submissions = all_submissions.where(status: ['送信失敗', 'CAPTCHAエラー', '送信状態不明']).count
    
    # Calculate success rate
    @success_rate = @total_attempts > 0 ? 
                   (@successful_submissions.to_f / @total_attempts * 100).round(1) : 
                   0
    
    # Track failure reasons for debugging
    @failure_breakdown = all_submissions
                        .where("status IN ('送信失敗', 'CAPTCHAエラー', '送信状態不明')")
                        .group(:status)
                        .count
  end
