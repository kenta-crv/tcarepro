...
class AutoformSchedulerWorker
  ...
  sidekiq_options queue: 'autoform_submissions', retry: 3

  PYTHON_SCRIPT_PATH = Rails.root.join('autoform', 'bootio.py').to_s
  PYTHON_EXECUTABLE = ENV['PYTHON_EXECUTABLE'] || %w[python3 python].find { |cmd| system("which #{cmd} > /dev/null 2>&1") } || 'python'
  ...
  def perform(contact_tracking_id)
    ...
    unless inquiry
      ...
      contact_tracking.update(status: '自動送信エラー', notes: error_message)
      return
    end
    ...
    unless PYTHON_EXECUTABLE && File.exist?(PYTHON_EXECUTABLE) || system("which #{PYTHON_EXECUTABLE} > /dev/null 2>&1")
        ...
        contact_tracking.update(status: '自動送信システムエラー', notes: "Python executable not found.")
    ...
    contact_tracking.update(status: '処理中', notes: 'Pythonスクリプト実行開始')
    ...
    begin
      ...
      if status.success?
        ...
        if current_status_after_script == '処理中'
            contact_tracking.update(status: '自動送信エラー', notes: "Python script finished but status not updated. Check script logic. STDERR: #{stderr.truncate(150)}")
        end
      else
        ...
        if current_status_after_script == '処理中'
          contact_tracking.update(status: '自動送信システムエラー', notes: "Python script execution failed (exit code #{status.exitstatus}). STDERR: #{stderr.truncate(150)}")
        end
      end
      ...
      if contact_tracking.status == '処理中' || contact_tracking.status == '自動送信予定'
        contact_tracking.update(status: '自動送信システムエラー', notes: "Ruby worker error: #{e.message.truncate(200)}")
      end
    end
  end
end