...
- **重要**: ダッシュボードでは制限に達していない（RPM: 4/15, TPM: 108K/1M）
- **結論**: 実際のクォータ超過ではなく、一時的なレート制限の可能性が高い
...
**実際の状況**:
- ダッシュボードでは制限に達していない（RPM: 4/15, TPM: 108K/1M）
- しかし、429エラー（ResourceExhausted）が発生している

**推定される原因**:
1. **一時的なレート制限（スライディングウィンドウ）**
   - Google APIは1分間のスライディングウィンドウでレート制限を適用
   - 短時間に複数のAPI呼び出しが集中すると、一時的に制限に達する可能性
   - ダッシュボードは1時間単位の集計のため、短時間のスパイクは見えない

2. **Google Searchツールの使用**
   - Google Searchツールを使用する場合、別のレート制限が適用される可能性
   - `node_get_url_candidates`でGoogle Searchツールを使用している

3. **エラーハンドリングの問題**
   - すべてのResourceExhaustedエラーを「QUOTA_EXCEEDED」として扱っていた
   - 実際には一時的なレート制限の可能性もある
